...
1. Produce your own findings on the target diff, in `schemas/findings.schema.json`
   shape (`file, line, severity, title, detail`). Write to a temp JSON file.
2. Get Codex's findings: `consort-review.sh` returns the same schema.
3. Merge: `node "$CLAUDE_PLUGIN_ROOT"/scripts/merge-findings.mjs claude.json codex.json`.
4. Present in this order: **Both agree** (act first), **Codex only** (what you missed,
